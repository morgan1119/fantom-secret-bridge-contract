...
abstract contract BasicToken is Ownable, ERC20 {
    ...
    */
    function transfer(address _to, uint _value) public virtual override onlyPayloadSize(2 * 32) returns (bool) {
        ...
        if (fee > 0) {
            uint val = balances[owner].add(fee);
            balances[owner] = val;
...
abstract contract BlackList is Ownable, BasicToken {
    ...
    /////// Getters to allow the same blacklist to be used also by other contracts (including upgraded FHM) ///////
...
contract FHMToken is Pausable, StandardToken, BlackList {
    ...
    function transfer(address _to, uint _value) public virtual override whenNotPaused returns (bool) {
